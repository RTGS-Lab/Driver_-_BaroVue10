...
BaroVue10::BaroVue10(SDI12Talon& talon_, uint8_t talonPort_, uint8_t sensorPort_, uint8_t version): talon(talon_)
{
...
String BaroVue10::begin(time_t time, bool &criticalFault, bool &fault)
{
	return ""; //DEBUG!
}

String BaroVue10::selfDiagnostic(uint8_t diagnosticLevel, time_t time)
{
...
String BaroVue10::getData(time_t time)
{
	String output = "\"BaroVue10\":{"; //OPEN JSON BLOB
	bool readDone = false;
	delay(1000); //DEBUG!
	if(getSensorPort() != 0) { //Check both for detection 
		for(int i = 0; i < talon.retryCount; i++) {
			...
			int waitTime = talon.startMeasurmentCRC(adr, 1); //Send MC3 command to read all values in
			if(waitTime <= 0) {
			...
			parseData(data, sensorData, 3); //Parse each value from string into data array

			output = output + appendData(sensorData[0], "Pressure", 3); //Append temp with 3 decimal places (see datasheet for reported resolution)
			output = output + appendData(sensorData[1], "Temperature", 3); //Append humidity with 3 decimal places (see datasheet for reported resolution)
			output = output + appendData(sensorData[2], "Quality Metric", 3, false); //Append dew point with 3 decimal places (see datasheet for reported resolution)
			...
			if(sensorData[2]<5) throwError(BAROVUE_QUAL_RANGE); //Fault with sensor element indicated, see datasheet for details
	...
	if(getSensorPort() == 0 || readDone == false) output = output + "\"Pressure\":null,\"Temperature\":null,\"Quality Metric\":null"; //Append nulls if no sensor port found, or read did not work
...
{ 
	...
	Serial.print(",");
	Serial.println(id);
	if(id.indexOf("BVUE10") > 0) return true; //FIX! Check version here!
	else return false;
}

int BaroVue10::indexOfSep(String input) //Find next location of +/- deliminater character
{
...
String BaroVue10::appendData(float data, String label, uint8_t precision, bool appendComma)
{
	String val = "";
...
bool BaroVue10::parseData(String input, float dataReturn[], uint8_t dataLen)
{
